...
/// 简化的技能管理器
/// </summary>
public class SimplifiedSkillManager : MonoBehaviour
{
    ...
    public bool CanCastSkill(int skillId)
    {
        ...
        // 获取角色属性
        var attributes = playerEntity.GetComponent<CharacterAttributes>();
        if (attributes == null) return false;

        // 检查基础状态
        if (!attributes.CanPerformAction("castskill")) return false;
        ...
        if (attributes.Mana < manaCost) return false;
    ...
    public async UniTask<bool> CastSkill(int skillId, Vector3 targetPosition, int targetId = 0)
    {
        ...
        attributes.ConsumeMana(manaCost);
    ...
    {
        // 添加防御Buff
        var buffManager = GetComponent<AttributeBasedBuffManager>();
        if (buffManager != null)
        {
